...
export default function Header() {
    ...
    return (
        <motion.header className="shadow-lg fixed left-1/2 sm:top-[2rem]  text-sm top-0 sm:w-[40rem] bg-white w-full sm:rounded-full z-[100]" initial={{ y: -110, x: "-50%", opacity: 0 }} animate={{ y: 3, x: "-50%", opacity: 0.95 }}>
            <ul className="flex flex-wrap items-center sm:justify-between justify-center w-full h-auto px-1 py-1 font-semibold transition-all box-border">
                {links.map((item) => (
                    ...
                    >
                        <motion.li className={`box-border text-gray-600 px-3 py-2 rounded-full hover:text-gray-900 transition ${activeState == item.hash && "bg-slate-200"}`}>{item.name}</motion.li>
                    </Link>
                ))}
            </ul>
        </motion.header>
