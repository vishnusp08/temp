...
export function useSectionInview({ thres, sectionTag }: Sometype) {
    ...
    useEffect(() => {
        if (inView && Date.now() - timeOfLastClick > 1000) setActiveState(sectionTag); // if no click is made on the header then the data.now will be more than 1000 hence the statement is executed any how. It is not executed when the click is made as the timeOfLastClick is almost 1000 seconds less
        // than the current time hence this doesn't get executed when click is made.
    }, [inView, setActiveState, timeOfLastClick, sectionTag]);

    return ref; //returning the reference pointer to the components so that it can be attached to the dom element that needs to be tracked in the viewport
}
